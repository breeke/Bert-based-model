...
\textbf{Multi-language real-world data.}
The most significant practical gap is the absence of a large-scale, function-level
real-world vulnerability dataset beyond C/C++.
DiverseVul provides approximately 18{,}000 labelled C functions; no comparable resource
exists for Python.
This forces synthetic training for Python and limits real-world evaluation to C, meaning
the synthetic-to-real gap measured in Experiment~3 cannot currently be replicated for
Python.
Constructing such a dataset --- sourced from PyPI security advisories, OSV database
entries, or GitHub advisory data --- and scaling the DiverseVul augmentation from
1{,}000 to 10{,}000 samples are the two highest-value data investments for future work.
The 0.278 F1 improvement from 1{,}000 real samples suggests the model responds
favourably to real data volume; closing the remaining gap is likely a data problem
