...
Hard negative mining addresses \emph{within-distribution confusion}: cases where the model
cannot discriminate between safe and vulnerable examples sharing the same high-level API
because the training data only presents the dangerous version.
This is a data coverage problem --- the test examples are stylistically identical to
training examples, but the model has no contrastive examples to anchor the decision boundary.
Adding contrastive pairs directly adjacent to the confusing examples reshapes the local
decision boundary without requiring architectural or training procedure changes.

Real-world data augmentation addresses \emph{out-of-distribution generalisation}: the gap
between clean, isolated, purpose-built training examples and the messy, long, context-dependent
functions that appear in real codebases.
No amount of carefully constructed hard negatives can teach the model what real-world C code
looks like if real-world C code has never appeared in training.
The augmentation directly reduces this covariate shift by exposing the model to the target
distribution, even with only 1{,}000 samples.

The residual gap of $-0.182$ after both interventions reflects factors that neither
intervention can address: CWE classes entirely absent from training (CWE-787, CWE-400,
CWE-703), architectural inability to track data flow for CWE-78 and CWE-416, and the
irreducible stylistic diversity of production code.
Closing the remaining gap would require substantially more real-world training data or
a move to a graph-based architecture --- directions explored in Chapter~\ref{ch:discussion}.
